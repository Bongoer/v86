...
function ___pthread_create_js(pthread_ptr, attr, startRoutine, arg) {
  ...
  for (var canvas of Object.values(offscreenCanvases)) {
    ...
    HEAPU32[(((canvas.canvasSharedPtr) + (8)) >>> 2) >>> 0] = pthread_ptr;
  }
  var threadParams = {
    startRoutine,
    pthread_ptr,
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 0, 1, fd, buf, len, flags, addr, addrlen);
  buf >>>= 0;
  len >>>= 0;
  addr >>>= 0;
  addrlen >>>= 0;
...
function ___syscall_recvmsg(fd, message, flags, d1, d2, d3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 0, 1, fd, message, flags, d1, d2, d3);
  message >>>= 0;
  try {
    var sock = getSocketFromFD(fd);
...
function ___syscall_sendmsg(fd, message, flags, d1, d2, d3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, fd, message, flags, d1, d2, d3);
  message >>>= 0;
  d1 >>>= 0;
  d2 >>>= 0;
  try {
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(36, 0, 1, fd, message, length, flags, addr, addr_len);
  message >>>= 0;
  length >>>= 0;
  addr >>>= 0;
  addr_len >>>= 0;
...
function ___syscall_socket(domain, type, protocol) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(37, 0, 1, domain, type, protocol);
  try {
...
var getCanvasSizeCallingThread = (target, width, height) => {
  ...
  if (canvas.canvasSharedPtr) {
    ...
    var w = HEAP32[((canvas.canvasSharedPtr) >>> 2) >>> 0];
    var h = HEAP32[(((canvas.canvasSharedPtr) + (4)) >>> 2) >>> 0];
    HEAP32[((width) >>> 2) >>> 0] = w;
    HEAP32[((height) >>> 2) >>> 0] = h;
  } else if (canvas.offscreenCanvas) {
    HEAP32[((width) >>> 2) >>> 0] = canvas.offscreenCanvas.width;
    HEAP32[((height) >>> 2) >>> 0] = canvas.offscreenCanvas.height;
...
function getCanvasSizeMainThread(target, width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(64, 0, 1, target, width, height);
...
var setCanvasElementSizeCallingThread = (target, width, height) => {
  ...
  if (canvas.canvasSharedPtr) {
    ...
    HEAP32[((canvas.canvasSharedPtr) >>> 2) >>> 0] = width;
    HEAP32[(((canvas.canvasSharedPtr) + (4)) >>> 2) >>> 0] = height;
  }
  if (canvas.offscreenCanvas || !canvas.controlTransferredOffscreen) {
    ...
    var targetThread = HEAPU32[(((canvas.canvasSharedPtr) + (8)) >>> 2) >>> 0];
...
function _emscripten_get_element_css_size(target, width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 0, 1, target, width, height);
  target >>>= 0;
  width >>>= 0;
  height >>>= 0;
...
function _emscripten_get_gamepad_status(index, gamepadState) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(70, 0, 1, index, gamepadState);
  gamepadState >>>= 0;
...
function _emscripten_get_screen_size(width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(72, 0, 1, width, height);
  width >>>= 0;
  height >>>= 0;
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(73, 0, 1, target, deferUntilInEventHandler, fullscreenStrategy);
  target >>>= 0;
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 0, 1, target, deferUntilInEventHandler);
  target >>>= 0;
  target = findEventTarget(target);
  if (!target) return -4;
  if (!target.requestPointerLock) {
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(76, 0, 1, userData, callbackfunc, targetThread);
  userData >>>= 0;
  callbackfunc >>>= 0;
  targetThread >>>= 0;
...
function _emscripten_set_element_css_size(target, width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(78, 0, 1, target, width, height);
  target >>>= 0;
  target = findEventTarget(target);
  if (!target) return -4;
...
function _emscripten_set_gamepadconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(81, 0, 1, userData, useCapture, callbackfunc, targetThread);
  userData >>>= 0;
  callbackfunc >>>= 0;
  targetThread >>>= 0;
...
function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(82, 0, 1, userData, useCapture, callbackfunc, targetThread);
  userData >>>= 0;
  callbackfunc >>>= 0;
  targetThread >>>= 0;
  if (_emscripten_sample_gamepad_data()) return -1;
...
function _emscripten_set_touchmove_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(95, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  target >>>= 0;
  userData >>>= 0;
  callbackfunc >>>= 0;
  targetThread >>>= 0;
...
function _emscripten_set_visibilitychange_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(97, 0, 1, userData, useCapture, callbackfunc, targetThread);
  userData >>>= 0;
  callbackfunc >>>= 0;
  targetThread >>>= 0;
...
function _emscripten_set_window_title(title) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 0, 1, title);
  title >>>= 0;
...
function _getaddrinfo(node, service, hint, out) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(110, 0, 1, node, service, hint, out);
  node >>>= 0;
  service >>>= 0;
  hint >>>= 0;
  out >>>= 0;
